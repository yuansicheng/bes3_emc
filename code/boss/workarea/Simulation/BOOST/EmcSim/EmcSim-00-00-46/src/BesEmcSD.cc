...
G4bool BesEmcSD::ProcessHits(G4Step* aStep,G4TouchableHistory*)
{
  ...
  //-----------------------------------------------------  
  // yuansc, 11.30
  static ISvcLocator* svcLocator = Gaudi::svcLocator();
  static INNGeneratorSvc* nn_generator_svc;
  StatusCode sc = svcLocator->service("NNGeneratorSvc", nn_generator_svc);
  if(sc != StatusCode::SUCCESS) {
    G4cout << "NNGeneratorSvc\t" << "EmcRec Error: Can't get NNGeneratorSvc." << G4endl;
  }

  if ((aStep->GetTrack()->GetDefinition()->GetParticleName() == nn_generator_svc->getTargetParticle().c_str()) && (PartId == 1)){
    // hit to emc barrel
    G4cout << "NNGeneratorSvc\t" << "hit emc barrel: " << nn_generator_svc->getTargetParticle().c_str() << G4endl;
    G4VUserTrackInformation* info = new G4VUserTrackInformation("1");
    aStep->GetTrack()->SetUserInformation(info);
    // if use nn, kill the track
    if (nn_generator_svc->getUseNN()){     
      G4cout << "NNGeneratorSvc\t" << "kill: " << nn_generator_svc->getTargetParticle().c_str() << G4endl;
