...
// yaunsc
void EmcRec::addNNHits(){
  MsgStream log(msgSvc(), name());

  static INNGeneratorSvc* nn_generator;
  StatusCode sc = service("NNGeneratorSvc", nn_generator);
  if(sc != StatusCode::SUCCESS) {
    log << MSG::ERROR << "EmcRec Error: Can't get NNGeneratorSvc." << endmsg;
  }

  if (!nn_generator->getUseNN()){
    log << MSG::DEBUG << "Do not use NN" << endmsg;
    return;
  }
  ...
  if (!mcParticleCol) {
    ...
    HepLorentzVector init_p;
    Hep3Vector final_pos;
    McParticleCol::iterator iter_mc = mcParticleCol->begin();
    for (;iter_mc != mcParticleCol->end(); iter_mc++) {
      if(!(((*iter_mc)->statusFlags() & 1<<13) >> 13)){
        continue;
      }
      init_p = (*iter_mc)->initialFourMomentum();
      final_pos = (*iter_mc)->finalPosition().v();
      // set init_p
      nn_generator->generate(init_p, final_pos);
      log << MSG::INFO << "nn_generator->generate(init_p, final_pos)" << endmsg;

      // get nn prediction and add to hitmap
      std::vector<EmcHit>* nn_emc_hit_col = nn_generator->getHitCol();

      for (int i = 0; i < nn_emc_hit_col->size(); i++){
        RecEmcHit aHit;
        aHit.CellId(EmcID::crystal_id((*nn_emc_hit_col)[i].global_part, (*nn_emc_hit_col)[i].global_ntheta, (*nn_emc_hit_col)[i].global_nphi));
        aHit.Energy((*nn_emc_hit_col)[i].energy);
        aHit.Time(0);
        fHitMap[aHit.getCellId()] = aHit;
      }
      log << MSG::INFO << "set nn hits to hitmap" << endmsg;
    }
  }
}
