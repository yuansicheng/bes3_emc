...
SIZES = [11, 11, 12, 12, 12, 6]
top_path = '/hpcfs/juno/junogpu/fangwx/FastSim/Others/caloflow/'
...
def plot_calo_batch(tensor, fp, layer, ncol=8, vmin=1e-2, vmax=None, save_it=True,
    ...
    plot_sizes = scale_factor * np.array([4, 4, 4, 4, 2, 4])
...
def plot_layer_energy(tensor, fp, layer, save_it=True, plot_ref=None, epoch_nr=None,
    ...
    if plot_ref is not None:
        ...
        if plot_ref in ['eplus', 'gamma', 'piplus']:
            reference_data = pd.read_hdf('{}plots_reference_{:1.0e}.hdf'.format(top_path,ref_thres))
    ...
    if plot_GAN is not None:
        if plot_GAN in ['eplus', 'gamma', 'piplus']:
            reference_data = pd.read_hdf('%s/plots_reference_CaloGAN.hdf'%top_path)
...
def plot_loss(train_loss, test_loss, fp, save_it=True):
    ...
    if len(train_arg) != len(train_loss):
        train_arg=train_arg[:len(train_loss)]##FIXME, fast fix, wxfang
